...
float voltageToDegrees(float value, float defaultWindDirection)
{
  ...
  // Note:  The documenation for the wind vane says 16 positions.  Only recieve 8 positions.  And 315 degrees is wrong.
...
extern uint8_t latest_interrupted_pin;
...
SDL_Weather_80422::SDL_Weather_80422(int pinAnem, int pinRain, int intAnem, int intRain, int ADChannel, int ADMode)
{
  ...
  pinMode(pinAnem, INPUT);	   // Pin A0 is input to which a switch is connected
  digitalWrite(pinAnem, HIGH);   // Configure internal pull-up resistor
  pinMode(pinRain, INPUT);	   // Pin A1 is input to which a switch is connected
...
float SDL_Weather_80422::current_wind_direction()
{
    ...
    float direction = voltageToDegrees(voltageValue, _currentWindDirection);
      // apply correction to Wind Vane since it is mounted backwards to N
